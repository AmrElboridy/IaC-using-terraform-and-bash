...
CHK2=$( sshpass -f password.txt  ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -p 22 root@vodafone-pgsql-1 "podman exec vodafone-pgsql-1 repmgr -f /opt/bitnami/repmgr/conf/repmgr.conf node check | grep 'node is primary' | wc -l")
        if [[ $CHK == 1 ]] && [[ $CHK2 == 3 ]]; 
        then
